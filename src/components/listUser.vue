    ...
    <div :class="[showLoadMore ? 'height600' : 'height628', 'lm_scroll listUserUlWrap']">
        <ul>
            <li 
                ...
                :key="item.id"
                :class="[isActive === item.friendEid ? 'bg_active' : '', item.top ? 'top_active' : '']"
                @click="handleListUserClick(item)"
            >
                <x-avatar :imgUrl="item.imgUrl" />
                <span v-html="item.remark ? item.remark : item.friendEid"></span>
                <div class="checkboxWrap" @click="handleSendToCheckMember(item)">
                    <slot></slot>
                </div>            
            </li>
        </ul>
        <div class="searchNoResult" v-show="checkMemberList.length < 1 && inputSearchVal !== ''">
